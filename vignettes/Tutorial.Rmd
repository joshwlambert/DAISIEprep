...
node_ids <- phylobase::nodeId(phylod)[which(
  phylobase::tdata(phylod)$island_status == "island"
)]

node_pies <- data.frame(
  asr$ancestral_likelihoods[, 1],
  asr$ancestral_likelihoods[, 2],
  node = node_ids)
...
ggtree::inset(tree_view = p, insets = pies, width = 0.1, height = 0.1)
```

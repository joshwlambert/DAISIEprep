...
## In this tutorial, the main features of `DAISIEprep` are explained. The objective is to identify island colonisation events from a time-calibrated phylogenetic tree, assign an island endemicity status (endemic, non-endemic, not present) to each of them, and then extract times of colonisation of the island and diversification within the island.
...
#### In this section we demonstrate a simple example of extracting and formatting data from a single phylogeny. We use a simulated phylogeny, which makes it clear how the data is structured. 
...
```{r}
phylo$tip.label <- c("Plant_a", "Plant_b", "Plant_c", "Plant_d", "Plant_e", "Plant_f",
                     "Plant_g", "Plant_h", "Plant_i", "Plant_j")
```
...
this should be based on the actual endemicity status of each tip!**).
...
algorithm. Each row in the `island_tbl` corresponds to a separate colonisation of the island. **In this case, two colonist lineages were identified using the 'min' extraction algorithm, one endemic and another non-endemic.**

# should we explain here the min algorithm?
...
To convert to the DAISIE data table we use `as_daisie_datatable()`, providing
...
# To discuss: I would skip the step below and the function create_daisie_data would use the island_tbl as input or at least hide it from the user. Also, the precise_col_time must apply to all lineages?
...
# Let's discuss this text below - I think this is an out-of-date-version
...
The final step is to produce the DAISIE data list, i.e. the input data of the DAISIE inference 
model. To do this we convert from the DAISIE data table produced in the last step, using the
`create_daisie_data()` function. Again, we provide
the island age (in the example, 1 million year), and this function requires the number of 
...
of the island colonist ([these are explained here if you are interested.](https://cran.r-project.org/web/packages/DAISIE/vignettes/stac_key.html)).
...
phylogeny and instead focus only the island species, which are usually the focus of
...
`island_tbl` for the *Coccyzus* tree.

# Change to asr? And maybe also show the ancestral states for Coccyzus
...
[See vignette on optimising parameters using DAISIE](https://cran.r-project.org/web/packages/DAISIE/vignettes/demo_optimize.html)
...
### 3.2 Adding a lineage when a phylogeny is not available for the lineage, but a colonisation time estimate exists
...
the island colonist is known from the literature. For illustrative purposes
...
# I have edited the text below - not sure if you use it elsewhere in the package
  ...
  * `missing_species` how many species are missing phylogenetic data from this 
  colonist. **In the case of a lineage with just 1 species (i.e. not an island radiation) 
  the number of missing species is zero as by adding the colonist it counts as
  one automatically*. 
  
# Not sure this is correct
  
  *However, in the case of an island clade of at least two 
  species the correct number of species missing from the clade should be given.** 
...
### 3.3 Adding a lineage when a phylogeny is not available for the lineage, and no colonisation estimate is available.

Taking the example above in `3.2`, but assuming we did not have any phylogenetic data about
...
# Not sure the example I added below is allowed

### 3.4 Adding a lineage when a phylogeny is not available for the entire island lineage, but a crown age or minimum colonisation time estimate exists

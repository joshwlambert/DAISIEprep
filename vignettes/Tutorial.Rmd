...
This will return a warning message for the Darwins finches as the root state of
the finches phylogeny is non-endemic to the island, as there is only a single
mainland outgroup in the phylogeny. This means that the colonisation time will
be extracted as infinite and then when the island_tbl is converted into a DAISIE
...
the island colonist is known from the literature. In this case, we need to input all the information on the lineage manually ourselves. For illustrative purposes

...
Just as `diversitree`, `corHMM` allows modelling and fitting SSE-class birth-death processes. 
Its specificity lies in the possibility to consider shifts between multiple, non-measurable regimes of evolution ("hidden" traits) controlling how the measured traits change and at what frequency.
This is particularly relevant for large or old clades, or any clade where it appears reasonable that not all parts of the tree have evolved under a unique process.

Let us consider the following example with our hypothetical bird clade, in the context island biogeography.
...
This difference is subtle and does not appear in the phylogenetic data.
...
```{r, sample_tree_corhmm, include=FALSE}
set.seed(
  2,
  kind = "Mersenne-Twister",
  normal.kind = "Inversion",
...
fit_corhmm
```
*Do mind the legend*: the numbering of trait states made by `corHMM` follows the order of first occurrence of each state in the data, and may differ from what you have used previously.
...
The tree suggests three miraculous not-present to endemic transitions!
...
# Extract posterior probabilities of each state for each node
asr_corhmm <- as.data.frame(fit_corhmm_hidden$states)
head(asr_corhmm) # 6 columns

# Merge (sum) probabilities for both regimes
for (i in 1:3) asr_corhmm[i] <- asr_corhmm[i] + asr_corhmm[i + 3]
asr_corhmm[4:6] <- NULL

...
![Visual representation of a typical dataset used in DAISIE analyses with two figures side-by-side. This representation uses a focal island community on the Hawaiian archipelago with nine species. Panel A shows a phylogeny with the island (Hawaiian) species highlighted, red for endemic, blue for non-endemic. P    anel B shows the separate phylogenies of those island species from the perspective of the archipelago. The island community (B) consists of lineages resulting from four colonisation events. Three of the island lineages contain endemic species and one contains only a single non-endemic species. Two of the endemic lineages have undergone cladogenetic speciation on the island, forming island clades with more than one species. The colonisation time is assumed to be the divergence time from the mainland sister species (stem age). Inset in Panel A is a world map, inset in Panel B is a map of Hawaii.](Figure1.png){width="674"}

...
The main appeal of the function is that it will automatically delineate
and extract the (paraphyletic) clades formed by island species within
the complete mainland + island phylogeny, and attempt to estimate the
age of colonisation for each of these clades, in a manner consistent
with DAISIE's assumptions.

However, there may be cases where it is not desirable to use this
...
the continent. This would result in a unique island clade with a unique
...
because DAISIE assumes no back-colonisation (from island to mainland),
such that the presence of mainland-only species e inside the island-only
clade can only be accomodated by the lineage staying on the mainland
...
from two geographic areas, which just happens to be the case of our

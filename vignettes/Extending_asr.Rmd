...
We do get a warning from `optimx`, but [apparently](http://phylo.wikidot.com/biogeobears-warnings-to-ignore-mostly#unused_control_arguments) this can be ignored.
...
Tips are mismatched smh.

relevant available options: stratification, area subsets, area-dependent effects etc.
DEC makes some assumptions like no widestate sympatry nor raage split but can be lifted in BGB.
Other processes can be turned off in the same way, by fixing parameters.

roxygen doc not compiled; read doc on source on github

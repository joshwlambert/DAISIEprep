...
path_to_phylo <- paste0(tempfile(pattern = "phylo_", fileext = ".txt"))
#system.file("extending_asr", "phylo.txt", package = "DAISIEprep", mustWork = TRUE)
path_to_biogeo <- tempfile(pattern = "biogeo_", fileext = ".txt")
...
Though I wasn't able to find documentation confirming this, marginal probabilities for the ancestral states appear to be found in element `$ML_marginal_prob_each_state_at_branch_top_AT_node` of the output, from what I infer from the code of the [plotting function code](https://github.com/nmatzke/BioGeoBEARS/blob/7b16e5263e91389d8e16b7bead4437d39b8be5bc/R/BioGeoBEARS_plots_v1.R#L432)).
The order of rows in this table correspond to the order of tips *in the tree object read from the input Newick file*, which may differ from the order in the original tree.
Columns respectively correspond to the first area (here, mainland, i.e. not present), second area (island, endemic), and the third column corresponds to the widespread state (nonendemic).
...
tail(asr_df)
```
As usual, we have wrapped this code in a utility, `extract_biogeobears_ancestral_states_probs()`.
...
`method` can also be set to `"random"` to sample states randomly based on the probabilities.

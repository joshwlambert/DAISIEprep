...
endemicity_status <- sse_states_to_endemicity(phylo$tip.state)
phylod <- phylobase::phylo4d(phylo, as.data.frame(endemicity_status))
plot_phylod(phylod)
```
...
```{r, create_musse_lik}
states <- endemicity_to_sse_states(phylod@data$endemicity_status)
names(states) <- phylod@label[1:phylobase::nTips(phylod)] # requires tip labels as names
...
simultaneously; colonisation of the mainland from the island are assumed
unlikely, and speciation on the island results in endemic lineages.
...
In fact, for modelling dispersal and evolution between mainland and
island, it is probably more appropriate to use an SSE model dedicated to
geographic traits.

...
#| fig.alt: >
#|   A scatter plot with error bars around the points. The y-axis is Mean run
#|   time (seconds) and the x-axis is Tree size (number of tips) on a log 
#|   scale. There is a legend titled Probability on the island with values
#|   0.2 (dark blue), 0.5 (gold), and DNA (pink) and complete (light blue).
#|   The dark blue and gold points increase roughly linearly with tree size,
#|   with gold points consistently slight greater than dark blue. The DNA and
#|   complete values only have a single point, but are below the trend for 
#|   dark blue and gold showing quicker run time for these datasets.
plot_performance(
...
#|   scale. There is a legend titled Probability on the island with values
#|   0.2 (dark blue), 0.5 (gold), and DNA (pink) and complete (light blue).
#|   The dark blue and gold points increase roughly exponentially with tree
#|   size, with dark blue points consistently greater than gold. The DNA and
#|   complete values only have a single point, but are below the trend for 
#|   dark blue and gold showing quicker run time for these datasets.
plot_performance(
...
#|   0.2 (dark blue), 0.5 (gold), and DNA (pink) and complete (light blue).
#|   The dark blue and gold points increase roughly linearly with tree size,
#|   with gold and dark blue points roughly equal. The DNA and complete values
#|   only have a single point, but are below the trend for dark blue and gold
#|   showing quicker run time for these datasets.
plot_performance(
  performance_data = performance_data$performance_data_min,
